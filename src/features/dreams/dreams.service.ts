...
function scheduleMockDreamProcessing(dreamId: string): void {
  setTimeout(() => {
    void processMockDream(dreamId).catch((error: unknown) => {
      console.error('[DREAM_MOCK_WORKER_ERROR]', error);
    });
  }, PROCESSING_DELAY_MS);
}

async function processMockDream(dreamId: string): Promise<void> {
  const now = new Date();
  ...
  await sleep(COMPLETION_DELAY_MS);
